...
include '../conf/koneksi.php';
    ...
    $kode_skdi = $_GET['kode_skdi'];
                 $sql = mysqli_query ($koneksi,"SELECT * FROM sk_domisili_i
                                                JOIN t_penduduk USING(id_penduduk) WHERE kode_skdi='$kode_skdi'");
                  while ( $data = mysqli_fetch_array($sql)) { /* Menampilkan Data Data*/ 
$pdf->SetTitle('SK Domisili ');
	...
	$pdf->Cell(20,1, "Surat Keterangan Domisili ", 0, 1, 'C');
	$pdf->Setfont("Arial", "", "10");	
	$pdf->Cell(20,0, "NOMOR : B-2975/Kk.17.02-1/Kp.08.2.".date('m')."/".date('Y'), 0, 1, 'C');
	$pdf->ln(0.5);	
	$pdf->Cell(5, 1, 'Yang bertanda tangan dibawah        :', 0, 1, '');
	$pdf->ln(0.9);
	...
	$pdf->ln(0.9);	
	$pdf->Cell(5, 0, '               Menurut Pengamatan Dan Data-data Kependudukan yang ada, Bahwa yang bersangkutan ', 0, 1, '');
	$pdf->Cell(5, 1, 'Memang Benar warga Desa Balida Kecamatan Paringin Kabupaten Balangan. Dan saat ini Berdomisili ', 0, 1, '');
	$pdf->Cell(5, 0, 'Dijalan Gunung Pandau Desa Balida Kecamatan Paringin Kabupaten Balangan ', 0, 1, '');
	$pdf->ln(0.9);
...
$pdf->Cell(5, 0, 'Demikian Surat ini dibuat semoga bisa dipergunakan sebagaimana mestinya .', 0, 1, '');
	$pdf->ln(0.9);
		$pdf->Setfont("Arial", "", "10");
		$pdf->Cell(5, 1, 'CATATAN .', 0, 1, '');
		$pdf->Cell(5, 1, '1. Surat keterangan Domisili ini berlaku selama 14 Hari setelah surat diterbitkan. .', 0, 1, '');
		$pdf->Cell(5, 0, '2. Jika laporan palsu, pelapor dapat dituntut sesuai dengan pasal 220  KUHP. .', 0, 1, '');
	$pdf->ln(0.9);
	...
	$pdf->ln(2);
	$pdf->Cell(30, 0.7, "SAHRIDIN", 0,1, "C");
